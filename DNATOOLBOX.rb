BASES=['A','T','C','G'];
YEASTBG ={"A"=>0.31, "T"=>0.31, "G"=>0.19, "C"=>0.19};
...
def loadValues(path)
	sequence=[nil]*2000000;
	require "GZ.rb"
	i=0;

	smartGZForeach(path){|line|
		if line[0,1]==">"
			next;
		end
		sequence[i]=line.chomp().to_f();
		i+=1;
	}
...
def loadSequence(path) #this is much faster, though ugly  it works by allocating memory in 10000 character chunks and then replacing it.
	require "GZ.rb"
	sequence="`"*10000;
	i=0;
	z=10000;
	smartGZForeach(path){|line|
		...
		line.chomp!();
		if i+line.length>z
			sequence=sequence+"`"*(line.length()+100000);
			z+=line.length()+100000;
		end
		sequence[i,line.length()] = line;
		i+=line.length();
	}
	sequence = sequence[0,i];
	return sequence.upcase();
end
...
def smartLoadPFMFromFile(inFP)
	begin
		tempPFM = loadPFMFromFile(inFP,true,false);
	rescue
		begin
			tempPFM = loadPFMFromFile(inFP,false,false);
		rescue
			begin
				tempPFM = loadPFMFromFile(inFP,true,true);
			rescue
				raise("UNKNOWN FORMAT FOR PFM: #{inFP}\n");
			end
		end
	end
	return tempPFM;
end
def getMaxPWMScore(pwm)
	#THIS ASSUMES IT IS A PWM ALREADY
	score=0.0;
	path="";
	0.upto(pwm["A"].length-1){|j|
		curBest=-999999.0;
		curBBase="Z";
		["A","T","G","C"].each{|base|
		  if pwm[base][j]>curBest
				curBest=pwm[base][j];
				curBBase=base;
			end
		}
...
class PFM
	...
	def trimByIC(threshold, background={"A"=>0.25, "T"=>0.25, "G"=>0.25, "C"=>0.25})
		#from front
		keepGoing=true;
		i=0;
		while i<self.length() && keepGoing
			#calculate IC
			ic = 0;
			background.keys().each(){|base|
				if @myPWM[base][i]==0
					...
					ic+=(@myPWM[base][i]*Math.log(@myPWM[base][i]/background[base]));
				end
			}
			if ic<threshold
				i+=1;
		...
		if i==self.length()
			warn("Warning: PFM trimmed down to nothing!\n");
			@myPWM = {"A"=>[], "T"=>[], "G"=>[], "C"=>[]};
			return;
		end
		#from back
		...
		while j>0 &&keepGoing
			...
			background.keys().each(){|base|
				if @myPWM[base][j]==0
					...
					ic+=(@myPWM[base][j]*Math.log(@myPWM[base][j]/background[base]));
				end
			}
			if ic<threshold
				j-=1;
		...
		newPFM = Hash.new();
		if j<i
			raise("Trimmed more from the top than was left from the bottom! i=#{i.to_s()} j=#{j.to_s()}\n");
		end
		warn("Trimming "+(i).to_s()+" from the start and "+(self.length()-1-j).to_s()+" from the end.");
		background.keys.each(){|base|
			newPFM[base] = @myPWM[base][i,j-i+1];
		}
		@myPWM = newPFM;
	end
	def normalize()
	...
	def to_IUPAC(iupacFP)
		...
		while iupacString.length()>0 && (iupacString[0,1].upcase()=="N" || iupacString[0,1].upcase()=="X")
			["A","T","G","C"].each{|base|
				newPFM[base].shift();
			}
		...
		while iupacString.length()>0 && (iupacString[-1,1].upcase()=="N" || iupacString[-1,1].upcase()=="X")
			["A","T","G","C"].each{|base|
				newPFM[base].pop();
			}
	...
	def percentOfMaxScan(seq, thresh=0)
		thePWM = self.toPWM();
		revPWM = self.revcomp().toPWM();
		return percentOfMaxScan_withWM(seq, thePWM, revPWM, thresh)
	end
	def percentOfMaxScan_PLPM(seq,thresh=0) #position log prob matrix
		thePWM = self.toPLPM();
		revPWM = self.revcomp().toPLPM();
		return percentOfMaxScan_withWM(seq, thePWM, revPWM, thresh)
	end
	def percentOfMaxScan_withWM(seq, thePWM, revPWM, thresh)
		starts = [];
		ends = [];
		hitScores =[];
		#calculateMaxScore
		maxScore, maxPath= getMaxPWMScore(thePWM);
		#maxScore=1 if maxScore==0;
		#maxScore= -maxScore if maxScore<0; #scale up to 0 if negative (PLPMs)
		0.upto(seq.length()-self.length()){|i|
			curScore = 0;
			0.upto(self.length-1){|j|
				curScore+=thePWM[seq[i+j,1]][j];
			}
			revScore =0;
			(self.length-1).downto(0){|j|
				revScore+=revPWM[seq[i+j,1]][j];
			}
			if maxScore<=0
				curScore=curScore-maxScore; #PLPM
				revScore=revScore-maxScore;
			else
				curScore=curScore/maxScore; #PWM
				revScore=revScore/maxScore; #PWM
			end
			if curScore>=thresh
				starts.push(i);
				ends.push(i+self.length()-1);
				hitScores.push(curScore);
			end
			if revScore>=thresh
				ends.push(i);
				starts.push(i+self.length()-1);
				hitScores.push(revScore);
			end
		}
		return [starts, ends, hitScores];
	end
	def scan(sequence)
		fScores= [];
		rScores= [];
	...
	def toPLPM() #in log probability, so 0 is the max, and -ve is bad
		newPWM = Hash.new();
		["A","T","C","G"].each{|base|
			newPWM[base]=[];
			@myPWM[base].each{|freq|
				if freq==0
					...
					newPWM[base].push((Math.log(freq)/Math.log(2)));
				end
			}
		}
		return newPWM;
	...
	def to_html()
	  returnMe="<table>";
		["A", "T", "G", "C"].each{|curBase|
			returnMe+="<tr><th>#{curBase}</th>";
	  	tempArray = @myPWM[curBase].map{|a| a.to_s}
	  	returnMe+="<td>"+tempArray.join("</td><td>")+"</td></tr>"
		}
		returnMe+="</table>\n";
		return returnMe;
	end
